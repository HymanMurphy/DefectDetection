...
class EnsembleModel:
    ...
    def ensemble_predict(self, pre1, pre2, pre3=None):
        ...
        if pre3 == None:
            if pre1[1][pre1[0].index('daisy')] > threshold and pre2[1][pre2[0].index('daisy')] > threshold:
                return True
            else:
                return False
        else:
            if pre1[1][pre1[0].index('daisy')] > threshold and pre2[1][pre2[0].index('daisy')] > threshold and pre3[1][pre3[0].index('daisy')] > threshold:
                return False
            else:
                return True
